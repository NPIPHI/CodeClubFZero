...
function makeCylinder(sides, height, bottomDiameter, topDiameter){
    let pos = [];
    let uvs = [];
    for(let i = 0; i < sides; i ++){
        let cos1 = Math.cos((i/sides)*Math.PI*2);
        let sin1 = Math.sin((i/sides)*Math.PI*2);
        let cos2 = Math.cos(((i+1)/sides)*Math.PI*2);
        let sin2 = Math.sin(((i+1)/sides)*Math.PI*2);
        pos.push(cos1*bottomDiameter);
        pos.push(0);
